...
29. 커널 및 모듈의 개념에 대하여 설명하세요. 
    커널 - 셸과 하드웨어 사이에서 하드웨어를 지원하기 위한 프로그램이다.
    모듈 - 커널의 하드웨어 지원 프로그램 중 사용 빈도가 적은 코드를 별도로 분리해 필요할 때마다 호출하여 사용하는 프로그램이다.
...
30. 커널 컴파일 순서에 대하여 설명하세요. 
    ...
    6) .config 편집 및 이전 정보 삭제 - 파일 편집 후 `make clean`
    7) 커널 컴파일 및 설치 - `make` -> `make modules_install`
       -> `make install` -> `ls -l /boot`
    8) 새로운 커널로 부팅 - `cat /etc/grub2/grub.cfg` -> `reboot`
